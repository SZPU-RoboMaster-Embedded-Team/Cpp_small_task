...
namespace HAL::CAN
{
    ID_t MCanDevice::extract_id(const CAN_RxHeaderTypeDef &rx_header)
    {
...
CanBus::CanBus() : can1_(&hcan1, 0, CAN_FILTER_FIFO1), can2_(&hcan2, 14, CAN_FILTER_FIFO0)   //列表式的构造函数初始化
      /*初始化CAN1 初始化CAN2*/  
{
    // 注册现有的设备
